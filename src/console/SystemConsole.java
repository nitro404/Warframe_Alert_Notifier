...
public class SystemConsole {
    ...
    private void writeLine(String text, boolean ignoreLog) {
		...
		while(m_consoleEntries.size() > AlertNotifier.settings.maxConsoleHistory) {
			m_consoleEntries.remove(0);
		}
		
		if(AlertNotifier.settings.logConsole && !ignoreLog) {
			addHeaderToLogFile();
    ...
    public boolean createLogDirectory() {
		if(AlertNotifier.settings.logDirectoryName.length() == 0) { return true; }
		
		File logDirectory = new File(AlertNotifier.settings.logDirectoryName);
		...
		if(!logDirectory.exists()) {
			try {
				...
				AlertNotifier.console.writeLine("Failed to create log directory, check read / write permissions.");
				return false;
			}
		}
    ...
    private boolean addHeaderToLogFile() {
		...
		File logFile = new File((AlertNotifier.settings.logDirectoryName.length() == 0 ? "" : AlertNotifier.settings.logDirectoryName + (AlertNotifier.settings.logDirectoryName.charAt(AlertNotifier.settings.logDirectoryName.length() - 1) == '/' || AlertNotifier.settings.logDirectoryName.charAt(AlertNotifier.settings.logDirectoryName.length() - 1) == '\\' ? "" : "/")) + AlertNotifier.settings.consoleLogFileName);
		...
		try {
			...
			out.println("[Console Output from " + Alert.LOG_DATE_FORMAT.format(Calendar.getInstance().getTime()) + "]");
		}
